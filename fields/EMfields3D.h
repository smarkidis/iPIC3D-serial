...
class EMfields3D : public Field{
 ...
 /** initialize the electromagnetic fields with constant values */
  ...
  /**  Init 3D null point(s) following Galsgaard & Nordlund (1997) */
  void initNullGalsgaard(VirtualTopology3D *vct, Grid *grid);
  /** initialized with rotated magnetic field */
  void initEM_rotate(VirtualTopology3D *vct, Grid *grid, double B, double theta);
  /**  Init Force Free (JxB=0) */
  void initRope(VirtualTopology3D *vct, Grid *grid);
  /** add a perturbattion to charge density */
  void AddPerturbationRho(double deltaBoB, double kx, double ky, double kz, double Bx_mod, double By_mod, double Bz_mod, double ne_mod, double ne_phase, double ni_mod, double ni_phase, double B0, Grid *grid);
  /** add a perturbattion to the EM field */
  void AddPerturbation(double deltaBoB, double kx, double ky, double kz, double Ex_mod, double Ex_phase, double Ey_mod, double Ey_phase, double Ez_mod, double Ez_phase, double Bx_mod, double Bx_phase, double By_mod, double By_phase, double Bz_mod, double Bz_phase, double B0, Grid *grid);
...
private:
  ...
  double *qom;
  /** thermal velocity for different species */
  double *uth, *vth, *wth;
...
inline EMfields3D::EMfields3D(CollectiveIO *col,Grid *grid){
  ...
  th = col->getTh();
  delt = c*th*dt;
  PoissonCorrection = col->getPoissonCorrection();
  CGtol = col->getCGtol();
...
/** init Flux rope */
inline void EMfields3D::initRope(VirtualTopology3D *vct, Grid *grid){
	// current along Y
	...
	double bth;
	double r,cth,sth;
    double ope_oce = sqrt(0.125);
	
	double ***vxn = newArr3(double,nxn,nyn,nzn);
	double ***vyn = newArr3(double,nxn,nyn,nzn);
	double ***vzn = newArr3(double,nxn,nyn,nzn);
	
	if (restart1 ==0){
        // initialize
        if (vct->getCartesian_rank() ==0){
            cout << "---------------------------------------------------" << endl;
            cout << "Initialize Rope Axis along Z" << endl;
            cout << "---------------------------------------------------" << endl;
            cout << "Bth0 (set in B0x)                = " << Bth0 << endl;
            cout << "B0z  (axial)                     = " << B0z << endl;
            cout << "w  = " << wid << endl;
        }
        for (int i=0; i < nxn; i++)
            for (int j=0; j < nyn; j++)
                for (int k=0; k < nzn; k++){
                    ...
                    for (int is=0; is < ns; is++){
                        rhons[is][i][j][k] = (rhoINIT[is]/FourPI); //*(1/((1 + r*r)*(1 + r*r)));
                    }
                    // Magnetic field
                    
                    cth = (grid->getXN(i,j,k)-x_center)/(r+1E-10);
                    sth = (grid->getYN(i,j,k)-y_center)/(r+1E-10);
                    // tangential component
                    bth = (Bth0*r)/(r*r+wid*wid +1E-10);
                    Bxn[i][j][k] = -bth*sth;
                    Byn[i][j][k] = bth*cth;
                    Bzn[i][j][k] = B0z;
        ...
        for (int i=0; i < nxc; i++)
            for (int j=0; j < nyc; j++)
                for (int k=0; k < nzc; k++){
                    r = sqrt((grid->getXC(i,j,k)-x_center)*(grid->getXC(i,j,k)-x_center) + (grid->getYC(i,j,k)-y_center)*(grid->getYC(i,j,k)-y_center));
                    // initialize the density for species
                    for (int is=0; is < ns; is++){
                        rhocs[is][i][j][k] = (rhoINIT[is]/FourPI); //*(1/((1 + r*r)*(1 + r*r)));
                    }
                    // Magnetic field
                    cth = (grid->getXC(i,j,k)-x_center)/(r+1E-10);
                    sth = (grid->getYC(i,j,k)-y_center)/(r+1E-10);
                    // tangential component
                    bth = (Bth0*r)/(r*r+wid*wid +1E-10);
                    Bxc[i][j][k] = -bth*sth;
                    Byc[i][j][k] = bth*cth;
		...
		// calculate J on centres and divide by 4 pi
		grid->curlC2N(vxn,vyn,vzn,Bxc,Byc,Bzc);
        // electric field
		for (int i=0; i < nxn; i++)
			for (int j=0; j < nyn; j++)
				for (int k=0; k < nzn; k++){
					Ex[i][j][k] = 0.0; //(c*vyn[i][j][k]/FourPI)*Bzn[i][j][k]/rhons[1][i][j][k] - (c*vzn[i][j][k]/FourPI)*Byn[i][j][k]/rhons[1][i][j][k];
					Ey[i][j][k] = 0.0; // (c*vzn[i][j][k]/FourPI)*Bxn[i][j][k]/rhons[1][i][j][k] - (c*vxn[i][j][k]/FourPI)*Bzn[i][j][k]/rhons[1][i][j][k];
					Ez[i][j][k] = 0.0; // (c*vxn[i][j][k]/FourPI)*Byn[i][j][k]/rhons[1][i][j][k] - (c*vyn[i][j][k]/FourPI)*Bxn[i][j][k]/rhons[1][i][j][k];
				}
        
        // calculate the the velocity and set in Z
        for (int i=0; i < nxn; i++)
            for (int j=0; j < nyn; j++)
                for (int k=0; k < nzn; k++){
                    for (int is=0; is < ns; is++){
                        
                        // in Jxs, Jys and Jzs you the fluid velocities
                        if (is==0){
                            Jxs[is][i][j][k] = -(1.0)*(c*vxn[i][j][k]/FourPI)/rhons[is][i][j][k]; // current only on electrons
                            Jys[is][i][j][k] = -(1.0)*(c*vyn[i][j][k]/FourPI)/rhons[is][i][j][k]; // current only on electrons
                            Jzs[is][i][j][k] = -(1.0)*(c*vzn[i][j][k]/FourPI)/rhons[is][i][j][k]; // current only on electrons
                            //if (Jzs[is][i][j][k]!=0)
                            //    cout << "Vze = "  << Jzs[is][i][j][k]<< endl;
                            
                        }
                        if (is==1){
                            Jxs[is][i][j][k] = (0.0)*(c*vxn[i][j][k]/FourPI)/rhons[is][i][j][k]; // no current on ions
                            Jys[is][i][j][k] = (0.0)*(c*vyn[i][j][k]/FourPI)/rhons[is][i][j][k]; // no current on ions
                            Jzs[is][i][j][k] = (0.0)*(c*vzn[i][j][k]/FourPI)/rhons[is][i][j][k]; // no current on ions
                            //if (Jzs[is][i][j][k]!=0)
                            //    cout << "Vzi = "  << Jzs[is][i][j][k]<< endl;
                        }
                    }
...
inline void EMfields3D::AddPerturbationRho(double deltaBoB, double kx, double ky, double kz, double Bx_mod, double By_mod, double Bz_mod, double ne_mod, double ne_phase, double ni_mod, double ni_phase, double B0, Grid * grid) {
  ...
  // cout<<" ne="<<ne_mod<<" ni="<<ni_mod<<" alpha="<<alpha<<endl;
    for (int i = 0; i < nxn; i++)
        for (int j = 0; j < nyn; j++)
            for (int k = 0; k < nzn; k++){
                rhons[0][i][j][k] += (ne_mod)* cos(kx * grid->getXN(i, j, k) + ky * grid->getYN(i, j, k) + kz * grid->getZN(i, j, k)  + ne_phase);
                rhons[1][i][j][k] += (ni_mod)* cos(kx * grid->getXN(i, j, k) + ky * grid->getYN(i, j, k) + kz * grid->getZN(i, j, k)  + ni_phase);
    }
...
inline void EMfields3D::AddPerturbation(double deltaBoB, double kx, double ky, double kz, double Ex_mod, double Ex_phase, double Ey_mod, double Ey_phase, double Ez_mod, double Ez_phase, double Bx_mod, double Bx_phase, double By_mod, double By_phase, double Bz_mod, double Bz_phase, double B0, Grid * grid) {
  ...
  for (int i = 0; i < nxn; i++)
    for (int j = 0; j < nyn; j++)
       for (int k = 0; k < nzn; k++){
      Ex[i][j][k] += Ex_mod * cos(kx * grid->getXN(i, j, k)  + ky * grid->getYN(i, j, k) + kz * grid->getZN(i, j, k) + Ex_phase);
      Ey[i][j][k] += Ey_mod * cos(kx * grid->getXN(i, j, k)  + ky * grid->getYN(i, j, k) + kz * grid->getZN(i, j, k) + Ey_phase);
      Ez[i][j][k] += Ez_mod * cos(kx * grid->getXN(i, j, k)  + ky * grid->getYN(i, j, k) + kz * grid->getZN(i, j, k) + Ez_phase);
      Bxn[i][j][k] += Bx_mod * cos(kx * grid->getXN(i, j, k) + ky * grid->getYN(i, j, k) + kz * grid->getZN(i, j, k) + Bx_phase);
      Byn[i][j][k] += By_mod * cos(kx * grid->getXN(i, j, k) + ky * grid->getYN(i, j, k) + kz * grid->getZN(i, j, k) + By_phase);
      Bzn[i][j][k] += Bz_mod * cos(kx * grid->getXN(i, j, k) + ky * grid->getYN(i, j, k) + kz * grid->getZN(i, j, k) + Bz_phase);
...
inline void EMfields3D::calculateHatFunctions(Grid * grid, VirtualTopology3D * vct) {
  ...
  communicateCenterBC_P(nxc,nyc,nzc,rhoc,2,2,2,2,2,2,vct);
  // calculate j hat
...
/**
*
*  Init 3D null point(s) following Galsgaard & Nordlund (1997), JGR 102, p. 231
*
*  (c) V. Olshevsky
*/
inline void EMfields3D::initNullGalsgaard(VirtualTopology3D *vct, Grid *grid){
	if (restart1 ==0){
		
		// initialize
		if (vct->getCartesian_rank() ==0){
			cout << "----------------------------------------" << endl;
			cout << "       Initialize 3D null point(s) Galsgaard & Nordlund variation" << endl;
			cout << "----------------------------------------" << endl;
			cout << "B0x                              = " << B0x << endl;
			cout << "B0y                              = " << B0y << endl;
			cout << "B0z                              = " << B0z << endl;
		...
		for (int j=0; j < nyn; j++)
		 for (int k=0; k < nzn; k++){
		   ...
		   for (int is=0; is < ns; is++){
				...
				Ez[i][j][k] =  0.0;
				// Magnetic field
				Bxn[i][j][k] = B0x*cos(2.*M_PI*grid->getYN(i,j,k)/Ly) - sin(2.*M_PI*grid->getZN(i,j,k)/Lz);
				Byn[i][j][k] = B0x*cos(2.*M_PI*grid->getZN(i,j,k)/Lz) - sin(2.*M_PI*grid->getXN(i,j,k)/Lx);
				Bzn[i][j][k] = B0x*cos(2.*M_PI*grid->getXN(i,j,k)/Lx) - sin(2.*M_PI*grid->getYN(i,j,k)/Ly);
			}
		   for (int i=0; i <nxc; i++)
			for (int j=0; j <nyc; j++)
			  for (int k=0; k <nzc; k++) {
				Bxc[i][j][k] = B0x*(cos(2.*M_PI*grid->getYC(i,j,k)/Ly) - sin(2.*M_PI*grid->getZC(i,j,k)/Lz));
				Byc[i][j][k] = B0x*(cos(2.*M_PI*grid->getZC(i,j,k)/Lz) - sin(2.*M_PI*grid->getXC(i,j,k)/Lx));
				Bzc[i][j][k] = B0x*(cos(2.*M_PI*grid->getXC(i,j,k)/Lx) - sin(2.*M_PI*grid->getYC(i,j,k)/Ly));
			}
		for (int is=0 ; is<ns; is++)
			grid->interpN2C(rhocs,is,rhons);
	} else {
	...
	if (restart1 ==0){
		...
		for (int j=0; j < nyn; j++)
		 for (int k=0; k < nzn; k++){
		   ...
		   for (int is=0; is < ns; is++){
					rhons[is][i][j][k] = rhoINIT[is]/FourPI;
                    //rhons[is][i][j][k] = 1./FourPI*(0.0004 - 0.5*(Bxn[i][j][k]*Bxn[i][j][k] + Byn[i][j][k]*Byn[i][j][k] + Bzn[i][j][k]*Bzn[i][j][k]))/((uth[0]*uth[0])/abs(qom[0]) + (uth[1]*uth[1]));
				}
				// electric field
				Ex[i][j][k] =  0.0;
				Ey[i][j][k] =  0.0;
...
inline double EMfields3D::getBenergy(void) {
  ...
  totalBenergy = localBenergy;
  return (totalBenergy);
}

/** get bulk kinetic energy*/
inline double EMfields3D::getBulkEnergy(int is) {
  double localBenergy = 0.0;
  double totalBenergy = 0.0;
  for (int i = 1; i < nxn - 2; i++)
    for (int j = 1; j < nyn - 2; j++)
